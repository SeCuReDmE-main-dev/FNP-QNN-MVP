...
Guardrail baseline: the full function-to-source registry now lives in `docs/source_ledger/math_function_source_guardrail.md`, with the organization source-of-truth baseline at `FNP-QNN-MVP-organisation/04_implementation_planning/MATH_SOURCE_FUNCTION_GUARDRAIL_BASELINE_2026-06-18.md`. That organization report also records the correct professor-facing Gmail thread context, GitHub commit evidence, and the current verified delta from the earlier 88-test summary to the 117-test baseline. After 2026-06-18, new source work should be appended as new source/function rows instead of repeating the whole history.
...
| [Foundation of Revolutionary Topologies](https://fs.unm.edu/TT/RevolutionaryTopologies.pdf) | Classical, neutro, and anti topology axiom framing; refined neutrosophic components; SuperHyper/nested topology; NonStandard monads/binads; Over/Under/Off and multiset topology. | `core/revolutionary_topologies.py` adds opt-in runtime topology profiles for axiom closure, deformation invariants, refined T/I/F components, neighborhood tolerance, nested metadata, and QNN feature additions when `revolutionary_topology_enabled=true`. | The simulator can expose bounded local topology-style structure metadata for education without changing default runtime behavior. |
| Local source: `[local maintainer path redacted]` | Plithogenic probability as multivariate event chances across variables; subclasses including neutrosophic, indeterminate, intuitionistic, picture/spherical, hybrid, and refined forms; multi-to-uni conversion by logical operators; plithogenic statistics as sample analysis. | `core/plithogenic_probability_statistics.py` adds deterministic empirical sample, probability-family, refined-statistics, multi-to-uni, and topology-completion profiles. The runtime bridge adds `plithogenic_topology` only when `plithogenic_enabled=true` and `revolutionary_topology_enabled=true`; the same profile is available at `/fnp-qnn/plithogenic-topology/runtime/profile`. | The simulator can statistically qualify plithogenic/topological runtime metadata for educational inspection without changing default runtime behavior. |
| [NeutroAlgebra is a Generalization of Partial Algebra](https://fs.unm.edu/NA/NeutroAlgebra.pdf) | Tri-sectioning into `A`, `neutroA`, and `antiA`; NeutroFunction; NeutroOperation; NeutroAxiom; NeutroAlgebra as a generalization of Partial Algebra. | `core/neutro_algebra.py` adds pure algebraic-integrity profiles and `neutroalgebra_runtime_profile()`. Runtime adds `neutro_algebra` only when `neutro_algebra_enabled=true`; the same profile is available at `/fnp-qnn/neutro-algebra/profile`. | The simulator can annotate whether runtime transformations and operation tables are inner-defined, indeterminate, outer-defined, or axiom-breaking without changing default runtime behavior. |
| [Structure, NeutroStructure, and AntiStructure in Science](https://fs.unm.edu/NA/NeutroStructure.pdf) | A structure is a non-empty space characterized by relations and attributes; relations and attributes may be classical, neutro, or anti; NeutroStructure and AntiStructure generalize classical structure. | `core/neutro_structure.py` adds relation, attribute, and system structural profiles. When `neutro_algebra_enabled=true`, runtime attaches `structure_system_profile` with `T_system/I_system/F_system`, LVFM attaches `neutro_structure_profile`, and QNN can receive the same bounded structural metadata. | The simulator can compute local structural health/readiness metrics from relations and attributes without treating them as clinical truth, production reliability, physical quantum validation, or a proof engine. |
| Local source: `[local maintainer path redacted]}} .pdf` | Narrative source for the deterministic baton/FfeD framing and bounded local carrier interpretation. | Supports the wording that `D_f_hat` is a local admissible carrier and not a production, security, or clinical claim. | The simulator can describe the baton/FfeD framing as local educational theory context only. |
| Local source: `[local maintainer path redacted]` | Fractal dimension grammar around `D_f`, `D_min`, `D_max`, normalized `D_f_hat`, and specialized `I_fractal`/`i_fractal` interpretation. | `normalize_fractal_dimension()` computes bounded `D_f_hat`; `fractal_carrier_profile()` returns `D_f_hat`, `dF_carrier`, `i_fractal_candidate`, hierarchy, and research boundary. | The simulator can expose a tested normalized fractal carrier while preserving `I -> I_system^S -> D_f -> dF -> i_fractal`. |
