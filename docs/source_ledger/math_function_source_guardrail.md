...
| `simulate_entangled_pair()`, `simulate_uncorrelated_probe()`, `simulate_local_coupling()`, `simulate_frustrated_state()`, `no_signalling_residual()`, `frustration_index()`, `gq_super_equation()`, `run_gravity_null_test()` | `core/gravity_null_test.py` | `VEDRAL_VIDEO_1_TESTING_QUANTUM_GRAVITY_REALITY`, `VEDRAL_VIDEO_2_THIS_QUANTUM_GRAVITY_EXPERIMENT`, `VEDRAL_OXFORD_PROFILE`, `FNG_GPCN_SET_PHI_FINAL_PDF`, `LOCAL_RUNTIME` | `GET /fnp-qnn/gravity-null-test/status`; `POST /fnp-qnn/gravity-null-test/run`; optional QNN smoke metadata when `gravity_null_test_enabled=true`; proof-of-principle simulator only, no physical quantum-gravity proof | `tests/test_gravity_null_test.py`, `tests/test_api_qnn_smoke.py` |
